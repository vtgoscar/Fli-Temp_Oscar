			...
			{#each servicesData as item}
				...
				>
					...
					<div class="content mt-6 relative z-1">
						<a href={item.link} class="title text-lg font-semibold hover:text-teal-500"
							>{item.title}</a
						>
						<p class="text-slate-400 mt-3">{item.desc}</p>

						<div class="mt-6">
							<a href={item.link} class="text-teal-500"
								>Read More <i class="mdi mdi-chevron-right align-middle"></i></a
							>
						</div>
					</div>
